...
def generate_rois_from_size(img_dims: Dimensions) -> np.ndarray[Any, np.dtype[ROI]]:
    ...
    n_horizontal = img_dims.width / ROI.WIDTH_PIXELS
    n_vertical = img_dims.height / ROI.HEIGHT_PIXELS
    ...
    if not n_horizontal.is_integer():
        warnings.warn(
            ...
            {ROI.WIDTH_PIXELS}. The remainder is {img_dims.width % ROI.WIDTH_PIXELS} pixels")
    if not n_vertical.is_integer():
        warnings.warn(
            ...
            of {ROI.HEIGHT_PIXELS}.  The remainder is {img_dims.height % ROI.HEIGHT_PIXELS} pixels")
