...
def cluster_rois(roi_similarity: pd.DataFrame, signals_arr: np.array, similarity_threshold: float, n_horizontal: int):
    ...
    rois_clusters_dict = {roi_linear_index_to_indexes(lin_idx, n_horizontal): cluster for lin_idx, cluster in
    ...
    for roi_idx, cluster_idx in enumerate(roi_cluster_associations):
        clusters[cluster_idx].append(roi_linear_index_to_indexes(roi_idx, n_horizontal))
